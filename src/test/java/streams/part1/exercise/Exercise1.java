...
class Exercise1 {
    ...
    void findPersonsEverWorkedInEpam() {
        ...
        List<Person> personsEverWorkedInEpam = employees.stream()
                .filter(jobHistoryEntries -> jobHistoryEntries.getJobHistory()
                        .stream()
                        .anyMatch(jobHistoryEntry -> jobHistoryEntry.getEmployer().equalsIgnoreCase("EPAM")))
    ...
    void findPersonsBeganCareerInEpam() {
        ...
        List<Person> startedFromEpam = employees
                .stream()
                .filter(e -> {
                    Optional<JobHistoryEntry> first = e.getJobHistory().stream().findFirst();
                    return first.isPresent() && first.get().getEmployer().equalsIgnoreCase("EPAM");
                })
                .map(Employee::getPerson)
                .collect(Collectors.toList());
